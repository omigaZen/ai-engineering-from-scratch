...
Docker 会把代码、运行时、库和系统工具打包成一个隔离单元，叫做容器。可以把它理解成轻量虚拟机，只是它共享宿主机的内核，所以启动只要几秒，不用等几分钟。
...
3. **多服务架构很常见。** 真实的 AI 应用不只是一个 Python 脚本。它可能还包括推理服务、用于 RAG 的向量数据库，也可能有一个 Web 前端。Docker Compose 可以用一条命令把这些服务都编排起来。
...
模型会保存在主机文件系统里。你可以随便重建容器，不用重新下载。
