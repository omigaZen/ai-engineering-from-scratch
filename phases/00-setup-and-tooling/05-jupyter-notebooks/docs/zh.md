...
每篇 AI 论文、教程、Kaggle 竞赛都会用到 Jupyter。你可以分块运行代码、内联查看输出、在文字与代码间切换、快速迭代。如果不使用 Notebook 学 AI，就像没有草稿纸做数学题。

但 Notebook 的坑也很多。人们往往把它用于本不该用的场景，后来才在调试里踩坑。知道什么时候用 Notebook、什么时候用脚本，可以避免很多后续灾难。
