...
范数是“向量有多大”的度量。任意两个向量的距离都可以写成它们差值的范数：`d(a, b) = ||a - b||`。所以理解范数就是理解距离。
...
和 L1 正则（Lasso）的关系：在损失中加入 `||w||_1` 会惩罚权重绝对值之和，使小权重被压到 0，完成自动特征选择。L1 约束在权重空间中呈菱形，角点对应某些维度权重为 0。
...
和 L2 正则（Ridge）关系：在损失里加入 `||w||_2^2` 惩罚大权重；与 L1 不同，它不会把权重推到 0，而是整体缩小。L2 约束是圆形区域，通常不在坐标轴上形成角，权重更小但很少精确为 0。

和损失函数关系：均方误差（MSE）是 L2 距离平方的平均。平方操作会比小误差更重惩罚大误差。

```text
MAE (L1 损失):  |y - y_hat|         线性惩罚。对离群点更鲁棒
MSE (L2 损失):  (y - y_hat)^2       二次惩罚。对离群点更敏感
```
...
欧氏距离可能认为两者离均值差不多远，
Mahalanobis 距离会把后者识别为离群点，因为它考虑了身高体重相关性。
```
...
```text
W(P, Q) = 对所有传输方案 gamma 的 inf  E[d(x, y)]
```
...
KL: infinity（因为 log(0)）
Wasserstein: 4（需将 1 的质量移动 4 个格子）

Wasserstein 有意义且有梯度，KL 不行。
```
...
MSE               L2 的平方                  大误差惩罚更重
MAE               L1                         对所有误差线性惩罚
Huber              小误差用 L2，大误差用 L1    折中方案，对异常值更稳健且零点近处平滑
交叉熵             KL 散度                    衡量分布不匹配
Hinge             max(0, margin - d)         只惩罚低于 margin 的部分
Triplet            L2（常见）                  拉近正样本，推远负样本
Contrastive        L2                         正样本更近，负样本超过 margin
```
...
```text
L1 正则（Lasso）:   loss + lambda * ||w||_1
  -> 权重稀疏，一部分权重恰为 0
  -> 自动特征选择
  -> 可行域是菱形，角点在坐标轴上

L2 正则（Ridge）:   loss + lambda * ||w||_2^2
  -> 权重偏小，整体收缩
  -> 不会产生稀疏特征（很少精确为 0）
  -> 解在全域平滑
...
```text
算法               思想                      常见使用
KD-tree            坐标轴对齐空间分割         scikit-learn（低维）
Ball tree          嵌套高维球                  scikit-learn（中维）
LSH                随机投影哈希               近似重复检测
HNSW               层次可导航小世界图          FAISS、Qdrant、Weaviate
IVF                倒排文件 + 聚类候选        FAISS（十亿级规模）
Product quant.     向量压缩后搜索             FAISS（内存受限）
```
