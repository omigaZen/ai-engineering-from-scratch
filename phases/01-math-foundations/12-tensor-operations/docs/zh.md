...
- 用 `einsum` 编写点积、矩阵乘、外积和批量运算表达式  
- 在多头注意力的每一步追踪完整的张量形状变化

## 问题

你正在实现一个 Transformer。前向过程写得很清楚，运行时报错：`RuntimeError: mat1 and mat2 shapes cannot be multiplied (32x768 and 512x768)`。你盯着形状看了半天，然后尝试转置，又出现 `Expected 4D input (got 3D input)`。你加了一个 `unsqueeze`，又是另一处报错。
...
矩阵只描述两个维度之间的关系，而真实数据常常超出二维。一个 `32` 张、`224x224` 的 RGB 图片 batch 是 `4D` 张量：`(32, 3, 224, 224)`。12 个注意力头的 self-attention 也是 `4D`：`(batch, heads, seq_len, head_dim)`。你需要一个能扩展到任意维度并在所有维度上统一组合的结构，这就是张量。把张量运算吃透，形状错误就会变得很容易定位。
...
总元素数是所有维度长度的乘积。形状 `(2, 3, 4)` 有 `2 * 3 * 4 = 24` 个元素。
...
`transpose` 不移动数据本身，它只交换步幅，张量会变成 **非连续** 的——同一行在内存中不再是连续存储。
...
关键写法：`i,i->`（点积）、`i,j->ij`（外积）、`ii->`（trace）、`ij->ji`（转置）、`bij,bjk->bik`（批量矩阵乘）、`bhtd,bhsd->bhts`（注意力分数）。
...
实现代码在 `code/tensors.py`，每步对应这里的内容。
...
对于 `(3, 4)` 的形状，步幅是 `(4, 1)`：行进一格跳 `4` 个元素，列进一格跳 `1` 个元素。

### 步骤 2：reshape、squeeze、unsqueeze

reshape 改变形状但不改动元素顺序。元素总数不变。`-1` 用于推断某一维大小。
...
`squeeze` 去掉大小为 1 的轴，`unsqueeze` 插入一个大小为 1 的轴。广播里经常要用：把偏置向量 `(D,)` 加到批量 `(B, T, D)`，通常先 `unsqueeze` 为 `(1, 1, D)`。
...
`transpose` 交换两条轴，`permute` 重新排列全部轴。NCHW 与 NHWC 的转换通常用它来完成。
...
转置或 permute 后张量通常变为非连续。PyTorch 中 `view` 在非连续时会失败，需改用 `reshape`，或先 `.contiguous()`。
...
卷积网络中的全局平均池化：`(B, C, H, W).mean(axis=[2, 3])` 得到 `(B, C)`。NLP 的序列平均池化：`(B, T, D).mean(axis=1)` 得到 `(B, D)`。

### 步骤 5：NumPy 广播演示

`tensors.py` 里的 `demo_broadcasting_numpy()` 演示了核心模式。
...
通过广播计算两组点对点距离时：把 `(M, 2)` reshape 成 `(M, 1, 2)`，把 `(N, 2)` reshape 成 `(1, N, 2)`，相减、平方、在最后一轴求和，再开平方，得到 `(M, N)`。

### 步骤 6：Einsum 运算

`demo_einsum()` 与 `demo_einsum_gallery()` 覆盖常见模式。
...
一次张量收缩的计算量是所有参与索引大小的乘积（保留轴和求和轴）。例如 `bij,bjk->bik`，若 B=32、I=128、J=64、K=128，则乘加次数是 `32 * 128 * 64 * 128 = 33,554,432`。

### 步骤 7：用 einsum 实现注意力

`demo_attention_einsum()` 给出多头注意力的端到端实现。
...
| Einsum | 暂无 | `np.einsum("ij,jk->ik", a, b)` |
...
1. **`outputs/prompt-tensor-shapes.md`**——用于系统排查张量形状不匹配的结构化提示词，包含常见操作（matmul、broadcast、cat、Linear、Conv2d、BatchNorm、softmax）的决策表和修复查找表。  
2. **`outputs/prompt-tensor-debugger.md`**——适用于 AI 助手的逐步排错提示词。给出报错信息和形状后，能返回精确修复步骤。

## 练习

1. **Easy — Reshape 往返**  
   构造形状 `(2, 3, 4)` 的张量。先 reshape 成 `(6, 4)`，再 `(24,)`，再还原为 `(2, 3, 4)`。每步检查扁平数据是否保持顺序一致。

2. **Medium — 实现广播**  
   给 `Tensor` 增加 `broadcast_to(shape)`，将维度 1 的轴扩展到目标形状。再让 `_elementwise_op` 在运算前自动广播。用 `(3, 1)` 和 `(1, 4)` 验证输出 `(3, 4)`。
