...
- 计算常见机器学习函数的数值导数与解析导数（如 `W1 = W1 - lr * dL/dW1`、sigmoid、交叉熵�?
...
导数描述“变化率”。对于函�?`W2 = W2 - lr * dL/dW2`，导�?`numerical_second_derivative(f, x)` 告诉你：如果�?`numerical_derivative` 稍微改动一点，y 会改动多少�?
...
�?x=2 时斜率是 4。若�?x 稍微向右移动，y 大约会增�?4 倍这段位移�? 
�?x=0 时斜率是 0，说明处在“碗底”�?
...
训练神经网络本质上是优化。你有一个损失函�?L(w1, w2, ..., wn) 来衡量模型错误程度，你要做的是最小化它�?
...
解析法：手工应用微积分规则。比�?f(x) = x^2，导数就�?f'(x)=2x，精确且快�?

数值法：用定义近似。取很小�?h 计算 f(x+h) �?f(x-h)�?
...
Hessian 是二阶偏导数组成的矩阵。对函数 f(x1, x2, ..., xn)，元�?(i, j) 为：
...
| Hessian 性质 | 含义 | 示例形状 |
|-------------|------|---------|
| 正定（全部特征�?> 0�?| 局部最小�?| 向上开的碗 |
| 负定（全部特征�?< 0�?| 局部最大�?| 向下开的碗 |
| 不定（正负特征值混合） | 鞍点 | 马鞍�?|
...
Newton 通常收敛更快，因�?Hessian 会“重标定”梯度——陡峭方向步长变小，平缓方向步长变大�?

但问题是：N 参数模型�?Hessian �?N x N�?00 万参数意味着 1 万亿项的矩阵，所以通常不可行，这就需要近似�?

| 方法 | 使用信息 | 计算代价 | 收敛特�?|
|------|----------|----------|---------|
| 梯度下降 | 一阶导�?| 每步 O(N) | 慢（线性） |
| 牛顿�?| 完整 Hessian | 每步 O(N^3) | 快（二次�?|
| L-BFGS | 用历史梯度近�?Hessian | 每步 O(N) | 中（超线性） |
| Adam | 每参数自适应学习率（近似对角 Hessian�?| 每步 O(N) | 中等 |
| 自然梯度 | Fisher 信息矩阵（统�?Hessian�?| 每步 O(N^2) | �?|
...
- **一阶泰�?= 梯度下降�?* 若用 f(x+h) ~ f(x) + f'(x)h，就是线性近似。梯度下降在这个模型下取 h = -lr * f'(x)�?
- **二阶泰勒 = 牛顿法�?* �?f(x+h) ~ f(x) + f'(x)h + 1/2 f''(x)h^2，得到二次模型，最小化�?h = -f'(x)/f''(x)，即牛顿步�?
...
每条边都乘以该边对应的局部导数。任一参数的梯度，是从损失到该参数路径上所有局部导数的乘积。路径分�?汇合时，需要把分支贡献相加（多变量链式法则）�?
...
你通常不会为神经网络手算雅可比，PyTorch 会替你完成。理解它有助于把握反向传播中的形状关系：如果一层映�?R^n -> R^m，它的雅可比�?m x n，梯度会经过其转置向后传播�?
...
鞍点函数�?Hessian 特征值是 2 �?-2（正负混合），说明是鞍点�? 
碗形函数�?Hessian 特征值都�?2，说明是最小点�?
...
�?x0=0 附近，sin(x) �?x（一阶泰勒）。当 h 很小近似非常好，h 大时会变差�? 
...
1. �?numerical_derivative 再调用一次，编写 numerical_second_derivative(f, x)。验�?x^3 �?x=2 的二阶导数是 12�?
2. 用梯度下降求 f(x, y) = (x - 3)^2 + (y + 1)^2 的最小值，起点 (0, 0)，应收敛�?(3, -1)�?
...
| 偏导�?| “某一个变量的导数�?| 在其他变量保持不变时，对某变量求�?|
| 梯度 | “最陡上升方向�?| 所有偏导数构成的向量，指向函数增长最快方�?|
| 梯度下降 | “往下走�?| �?参数 -= lr * 梯度 降低损失，神经网络训练核�?|
| 学习�?| “步长�?| 决定每步下降幅度，过大会发散，过小会收敛很慢 |
| 链式法则 | “导数乘法�?| 复合函数求导规则：df/dx = df/dg * dg/dx，是反向传播数学基础 |
| 雅可�?| “导数矩阵�?| 向量到向量映射时，输出对输入的全部偏导数组成矩阵 |
| 数值导�?| “有限差分�?| 用两个相邻点函数值计算斜率近似导�?|
| 反向传播 | “反向自动微分�?| 从输出到输入逐层应用链式法则求梯度，网络学习的核�?|
| Hessian | “二阶导数矩阵�?| 全部二阶偏导数组成的矩阵，描述函数曲率；在临界点正定表示局部最�?|
| 泰勒级数 | “多项式近似�?| 用导数在某点附近近似函数：f(x+h) ~ f(x) + f'(x)h + 1/2 f''(x)h^2 + ...，解释为何梯度下降与牛顿法成�?|
| 积分 | “曲线下的面积�?| 在区间上累积量，�?ML 中用于概率、期望和 KL 散度 |

## 延伸阅读

- [3Blue1Brown: Essence of Calculus](https://www.3blue1brown.com/topics/calculus) - 视觉化地理解导数、积分与链式法则
- [Stanford CS231n: Backpropagation](https://cs231n.github.io/optimization-2/) - 理解梯度如何在神经网络层中传�?
