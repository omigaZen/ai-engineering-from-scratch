...
每次训练线性回归，你都在解线性方程组。每次做最小二乘拟合，你都在解线性方程组。每次神经网络层计算 `y = Wx + b`，都是在执行线性方程组的一部分。加入正则化，就是在修改这个系统。使用高斯过程时，你要分解矩阵；在计算 Mahalanobis 距离时，你要逆协方差矩阵，也是在线性方程组上做计算。

`Ax = b` 这个式子到处都在出现。A 是已知系数矩阵，b 是已知输出向量，x 是你要寻找的未知量。在线性回归里，A 是数据矩阵，b 是目标向量，x 是权重向量。整个模型最后都会落到同一件事上：找到一个 x，使得 `Ax` 尽量接近 b。
...
高斯消元的复杂度是 O(n^3)。对于一个 1000x1000 的系统，这大约是一十亿次浮点运算。它已经很快了，但如果你需要用同一个 A 去解多个不同的 b，还能更省。
