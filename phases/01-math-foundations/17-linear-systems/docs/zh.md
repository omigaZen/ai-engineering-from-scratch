...
> 求解 `Ax = b` 是最古老的数学问题，如今它仍在运行你的神经网络。
...
每次训练线性回归你都在解线性系统；每次做 least squares 都在解线性系统；每次神经网络层算 `y = Wx + b` 都是在评估一组线性方程。加正则化就是改系统；高斯过程要做矩阵分解；Mahalanobis 距离要逆协方差矩阵，也是在解方程。

形式上我们解的是 `Ax = b`：`A` 是已知系数矩阵，`b` 是已知输出，`x` 是未知变量。在线性回归里，`A` 是数据矩阵，`b` 是目标向量，`x` 是权重向量。问题本质是：找一个 `x`，使 `Ax` 尽可能接近 `b`。
