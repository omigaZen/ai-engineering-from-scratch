...
> 浮点数是一种有漏洞的抽象。训练时它会咬你一口，而且你往往看不见。
...
- 识别浮点计算中的上溢、下溢和灾难性消除
- 用中心差分验证解析梯度和数值梯度
- 解释为什么训练时 bfloat16 通常优于 float16，以及损失缩放如何防止梯度下溢

## 问题

模型训练了 3 个小时，然后 loss 变成了 NaN。你加了一条打印语句。第 9,000 步时 logits 还是正常的；第 9,001 步它们变成了 `inf`；到第 9,002 步，所有梯度都成了 `nan`，训练当场死亡。

或者：模型训练完了，但准确率比论文低 2%。你把所有东西都检查了一遍。架构一致，超参数一致，数据也一致。问题在于论文用的是 float32，而你用了 float16，还没有做正确的缩放。32 位累积舍入误差悄悄吃掉了准确率。

或者：你从零实现了交叉熵损失。小 logits 时一切正常；当 logits 超过 100 时，结果变成 `inf`。softmax 溢出了，因为 `exp(100)` 已经超出了 float32 的表示范围。所有主流框架都用两行技巧解决这个问题，但你之前不知道这招。

数值稳定性不是理论问题，而是训练能否成功和悄悄失败的分水岭。你最终要调的很多严重 ML bug，本质上都来自浮点数。
...
float32 大约有 7 位十进制精度。这意味着它能区分 `1.0000001` 和 `1.0000002`，但分不清 `1.00000001` 和 `1.00000002`。超过 7 位之后，剩下的主要就是舍入噪声。

float16 只有大约 3 位精度。它能表示的最大数是 65,504。对机器学习里经常出现的 logits、梯度和激活来说，这个范围小得吓人。

bfloat16 是 Google 对 float16 范围问题的回答。它和 float32 有相同的 8 位指数，所以范围和 float32 一样大（最高约 3.4e38），但尾数只有 7 位。训练神经网络时，范围通常比精度更重要，所以 bfloat16 往往更适合训练。

### 为什么 `0.1 + 0.2 != 0.3`

0.1 在二进制浮点里不能精确表示。以 base-2 看，它是一个无限循环小数：
...
修复方式是重排公式，避免减去“几乎相等的大数”。方差可以用 Welford 算法，或者先把数据中心化。对数概率则尽量全程在 log 空间里处理。
...
为什么它有效：减去 `max(x)` 后，最大的指数会变成 `exp(0) = 1`，不会上溢。并且和里至少有一项是 1，所以总和至少是 1，`log(1) = 0`，也不会下溢成 `-inf`。
...
| 机器 epsilon | “精度边界” | 在某种浮点格式下，最小的 `e`，使得 `1.0 + e != 1.0`。float32 下约为 `1.19e-7`。 |
| 灾难性消除 | “相减导致精度丢失” | 两个很接近的浮点数相减时，有效数字被抵消，舍入噪声主导结果。 |
