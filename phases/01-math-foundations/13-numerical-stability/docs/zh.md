...
```text
格式       位数   Exponent  Mantissa  Decimal digits  Range（近似）
...
float32 大约有 7 位十进制有效数字，能区分 `1.0000001` 和 `1.0000002`，但区分不到 `1.00000001` 和 `1.00000002`。超过 7 位之后就变成舍入噪声。
...
十进制数 `0.1` 在二进制浮点中不能精确表示。以 base-2 表示是一个无限循环小数：

```text
0.1 in binary = 0.0001100110011001100110011...（无限循环）
```

float32 会截断到 23 位尾数。实际存储值大约是 `0.100000001490116`，同理 `0.2` 大约是 `0.200000002980232`。它们之和是 `0.300000004470348`，不是 `0.3`。
...
1. 像 `if loss < threshold` 这类比较可能给出错误判断  
2. 对大量小值累加（几千步的梯度更新）会漂移出真实和  
3. 校验和与可复现实验中若直接用 `==` 比较 float 往往失败  

修复方式：不要用 `==` 比较浮点数。用 `abs(a - b) < epsilon` 或 `math.isclose()`。
...
```text
a = 1.0000001    （float32 存储为 1.00000011920929）
...
- 数据方差计算且均值很大时：`E[x^2] - E[x]^2`，且 `E[x]` 很大  
- 两个几乎相等的对数概率相减  
...
Float32 边界:
  ...
  最小正非正规数: 1.401e-45
  上溢:   > 3.4e38 的值会变成 inf
  下溢:   < 1.4e-45 的值会变成 0.0
```

ML 里最常见的溢出源头是 `exp()`：

```text
exp(88.7)  = 3.40e+38   （刚好接近 float32 上限）
exp(89.0)  = inf         （上溢）
exp(-87.3) = 1.18e-38   （略高于下溢界）
exp(-104)  = 0.0         （下溢到 0）
```
...
log(-1.0)  = nan
log(1e-45) = -103.3      （可用）
log(1e-46) = -inf        （输入已下溢为 0，log(0) = -inf）
```

在 ML 中，`exp()` 出现在 softmax、sigmoid、概率计算；`log()` 出现在交叉熵、对数似然、KL 散度。`log(exp(x))` 这条链条在没有正确技巧时是“陷阱组合”。

### Log-Sum-Exp 技巧

直接算 `log(sum(exp(x_i)))` 非常危险。任何较大的 `x_i` 都可能让 `exp(x_i)` 上溢；若 `x_i` 都很小（负且大幅度），`exp(x_i)` 可能全变成 0，随后出现 `log(0) = -inf`。
...
为何有效：减去 `max(x)` 后最大指数是 `exp(0)=1`，因此不可能上溢。至少有一项为 1，使得和 >= 1，`log(1)=0`，因此不会因为下溢落到 `-inf`。
...
这些真的超过 float32 上限（约 3.4e38）吗？其实：
exp(88.7) 已触碰 float32 边界。
...
`nan`（不是一个数）和 `inf`（无穷）在计算中会级联传播。梯度更新中只要出现一个 `nan`，权重就会变成 `nan`，下一步输出也都 `nan`，训练一旦发生就会死掉。
...
math.isnan(x)       # x 为 nan 时 True
math.isinf(x)       # x 为 +inf 或 -inf 时 True
math.isfinite(x)    # x 既不是 nan 也不是 inf 时 True
```

防范策略：

1. 限定 `exp()` 输入：`exp(clamp(x, -80, 80))`
2. 分母加 epsilon：`x / (y + 1e-8)`
3. `log()` 内加 epsilon：`log(x + 1e-8)`
4. 用稳定实现（log-sum-exp、稳定版 softmax）
5. 梯度裁剪防止权重发散
6. 调试阶段每次前向后检查 `nan`/`inf`
...
它的误差是 `O(h^2)`，显著优于前向差分 `(f(x+h) - f(x)) / h` 的 `O(h)`。

步长选择：太大会导致近似不准，太小则灾难性消除主导了结果。通常 `h = 1e-5` 到 `1e-7`。
...
实现新层或损失时要始终做梯度检查。PyTorch 提供了 `torch.autograd.gradcheck()`。
...
1. 维护 float32 的参数主副本
2. 前向用 float16（更快）
3. 损失用 float32 计算（避免上溢）
4. 反向用 float16（更快）
5. 将梯度转换为 float32 并做缩放
6. 用 float32 主副本更新参数
```

纯 float16 训练的常见问题是梯度通常很小（1e-8 或更小），float16 会将低于约 `6e-8` 的值下溢为 0。模型因此“停学习”，因为梯度更新基本全为 0。

解决方法是损失缩放：

```text
1. 将损失乘以一个大系数（例如 1024）
2. 反向计算 `(loss * 1024)` 的梯度
3. 所有梯度都变成 1024 倍（抬高到 float16 可表示范围）
4. 更新参数前再除以 1024
5. 最终更新等价，且避免了下溢
```

动态损失缩放可自动调整该系数：初始值较大（如 65536）；若梯度出现 `inf`，则减半；若连续 N 步无溢出，则翻倍。
...
可保留梯度方向。PyTorch 的 `torch.nn.utils.clip_grad_norm_()` 即是该方式。它通常是更标准的选择。
...
其中 `epsilon`（通常是 1e-5）防止激活全相等时除零。可学习参数 `gamma`、`beta` 允许网络恢复任意需要的尺度。
