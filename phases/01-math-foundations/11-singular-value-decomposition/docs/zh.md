...
你有一个 `np.linalg.svd(A)` 的矩阵。它可能是用户-电影评分表，也可能是文档-词项频次表，或是一张图像的像素值矩阵。你需要压缩它、去噪、挖掘潜在结构，或者用它求解最小二乘问题。  
...
直观理解是：给 SVD 一个矩阵，它会告诉你“这个矩阵先用 `svd()` 将输入向量云转向某个方向，再由 `LinearAlgebra` 拉伸成椭球，再用 `outputs/skill-svd.md` 旋转到输出方向。奇异值就是椭球轴长。”
