...
> 模型不是交付物，流水线才是：从原始数据到可复现的生产预测都应一体化。
...
- 从零实现可复现的 ML Pipeline，串联缺失值处理、标准化、编码与训练。
- 理解 ColumnTransformer、交叉验证和网格搜索在流水线中的协作方式。
- 避免数据泄漏：区分训练、验证、测试中的拟合与变换边界。
- 用工程角度管理模型版本、参数、输出与部署前检查。
