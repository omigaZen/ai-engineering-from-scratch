...
> 把一组弱学习器以合适方式叠在一起，就能变成一组强模型。这个结论不是比喻。
...
- 从零实现 AdaBoost 与梯度提升树，理解其逐步修正残差的机制。
- 对比 Bagging 与 Boosting 的训练范式：并行降方差与序列降偏差。
- 理解随机森林和提升树在偏差-方差曲线中的不同表现。
- 掌握学习率、树深度、子采样比例等超参数对迭代收敛与泛化的影响。
