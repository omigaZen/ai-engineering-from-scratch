...
**类型:** Build  
**语言:** Python  
**先修:** 第 1 期（概率与分布、统计学），第 2 期第 1-8 课  
**时长:** ~90 分钟

## 学习目标

- 从零实现 K 折与分层 K 折交叉验证，理解分层在不平衡场景下为何关键。
- 实现分类与回归常见指标，并掌握适用边界：准确率、精确率、召回率、F1、AUC。
- 用学习曲线、验证曲线识别过拟合与欠拟合并定位调参方向。
- 理解训练/验证/测试集划分，避免数据泄漏并保证评估结论可复现。
