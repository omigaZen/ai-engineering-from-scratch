...
JAX 可以看作带着三种超能力的 NumPy：自动微分、编译到 XLA 的 JIT，以及自动矢量化。你写一个处理单个样本的函数，JAX 会帮你把它扩展成能处理批量、计算梯度、编译成机器码并跨多设备运行的版本，而且原始函数本身不用改。
...
`grad` 接受一个函数，并返回一个能计算梯度的新函数。这里没有 `torch.autograd.functional.hessian` 那种调用方式，张量里也不会保存计算图。梯度本身只是另一个函数，你可以继续调用、组合或用 JIT 编译它。
...
| 库 | 角色 | 风格 |
|--------|------|--------|
| **Flax**（Google）|神经网络层 |具有显式状态的 `self` |
