...
您知道如何在 PyTorch 中构建神经网络。您定义 `nn.Module`，调用 `.backward()`，步进优化器。有用。数百万人使用它。

但 PyTorch 的 DNA 中有一个限制：它热切地跟踪 Python 中的操作，一次一个。每个 `tensor + tensor` 都是一个单独的内核启动。每个训练步骤都会重新解释相同的 Python 代码。在您需要在 2,048 个 TPU 上训练 5400 亿个参数的模型之前，这种方法效果很好。那么开销就会杀死你。

Google DeepMind 在 JAX 上训练 Gemini。人类在 JAX 上训练了克劳德。这些都不是小规模的操作——它们是地球上最大的神经网络训练运行。他们选择 JAX 是因为它将您的训练循环视为可编译程序，而不是 Python 调用序列。
