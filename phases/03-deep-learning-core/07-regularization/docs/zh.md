...
graph TD
    subgraph "批量归一化"
        BN_D["按批次归一化<br/>每个特征分别处理"]
        BN_S["Batch: [x1, x2, x3, x4]<br/>Feature 1: normalize [x1f1, x2f1, x3f1, x4f1]"]
        BN_P["需要 batch > 32<br/>训练和推理不同<br/>用于 CNN"]
    end
    subgraph "层归一化"
        LN_D["按特征归一化<br/>每个样本分别处理"]
        LN_S["Sample x1: normalize [f1, f2, f3, f4]"]
        LN_P["不依赖 batch<br/>训练和推理一致<br/>用于 Transformer"]
    end
    subgraph "RMS 归一化"
...
最简单的正则化器：当验证损失开始增加时停止训练。此时模型尚未过度拟合。在实践中，你可以跟踪每个训练轮次的验证损失，保存最佳模型，并继续训练“耐心”窗口（通常为 5-20 轮）。如果验证损失在耐心窗口内没有改善，就停止并加载保存的最佳模型。
