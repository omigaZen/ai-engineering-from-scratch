...
你已经了解向量和点积，也知道矩阵可以把输入变成输出。但机器究竟是怎么“学会”该用哪种变换的？

感知机回答了这个问题。它是最简单的学习机器：拿输入、乘权重、加偏置，然后做二分类判断。接着根据结果调整参数。就这么简单。所有神经网络，本质上都是把这个想法一层层堆起来。
...
阶跃函数非常直接：如果加权和加偏置大于等于 0，就输出 1；否则输出 0。
...
AND 和 OR 都是线性可分的：你可以用一条直线把 0 和 1 分开。XOR 不是。没有一条直线能把 `[0,1]` 和 `[1,0]` 从 `[0,0]` 与 `[1,1]` 中分开。
...
解决办法是把感知机叠成层。多层感知机可以把两个线性决策组合成一个非线性决策，从而解决 XOR。
...
| 偏置 | “阈值” | 一个常数项，用来平移决策边界，让感知机在输入全为 0 时也能触发 |
| 激活函数 | “压缩数值的东西” | 加权求和后再应用的函数。感知机用阶跃函数，现代网络常用 sigmoid/ReLU |
| 线性可分 | “能画一条线分开” | 数据集可以被一个超平面完美分成两类 |
