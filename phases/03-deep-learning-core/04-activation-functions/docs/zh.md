...
> 没有非线性，你的 100 层网络只是一个花哨的矩阵乘法。激活函数就是让神经网络能“拐弯思考”的那道门。
...
激活函数打破了线性性。它们通过非线性函数扭曲每层的输出，让网络能弯曲决策边界、逼近任意函数，并真正学习复杂模式。但如果激活选错了，梯度就会在深层网络里消失到接近 0（sigmoid），或者在没有谨慎初始化时发散到无穷大，或者神经元会永久死亡（大负偏置下的 ReLU）。激活函数的选择，会直接决定你的网络能不能学。
...
代入就不再能化成单个线性变换了。网络因此能表示非线性函数。每加一层激活函数，表达能力也会更强。
...
输出范围是 `(-1, 1)`，以 0 为中心，因此有助于消除 sigmoid 的一部分抖动问题。
...
对正输入来说没有梯度消失。梯度就是 1，直接传过去。这就是深层网络开始真正可训练的原因。ReLU 能保持梯度在层间的量级。
...
graph LR
    subgraph "激活函数"
        S["Sigmoid<br/>范围： (0,1)<br/>两端会饱和"]
        T["Tanh<br/>范围： (-1,1)<br/>以 0 为中心"]
        R["ReLU<br/>范围： [0,inf)<br/>死亡神经元"]
        G["GELU<br/>范围： ~(-0.17,inf)<br/>平滑门控"]
    end
    S -->|"梯度消失"| Problem["深层网络<br/>训练不动"]
    T -->|"没那么严重，但<br/>还是会消失"| Problem
    R -->|"x > 0 时梯度 = 1"| Solution["深层网络<br/>训练更快"]
    G -->|"处处平滑的梯度"| Solution
```
...
flowchart TD
    ...
    Hidden -->|"隐藏层"| Arch{"架构是什么？"}
    Hidden -->|"输出层"| Task{"任务类型是什么？"}
