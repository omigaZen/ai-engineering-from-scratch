    ...
    def check_activations(self):
        ...
        for name, stats in self.activation_stats.items():
            if stats["fraction_zero"] > 0.5:
                issues.append(f"死神经元：{name} 有 {stats['fraction_zero']:.0%} 个激活为 0")
            if abs(stats["mean"]) > 10:
                issues.append(f"激活爆炸：{name} mean={stats['mean']:.2f}")
            if stats["std"] < 1e-6:
                issues.append(f"激活塌缩：{name} std={stats['std']:.2e}")
    ...
    def check_gradients(self):
        ...
        if len(grad_magnitudes) >= 2:
            ...
            if last_mag > 0 and first_mag / last_mag > 100:
                issues.append(f"梯度比值：first/last = {first_mag/last_mag:.0f}x（消失）")
        return issues if issues else ["HEALTHY"]
