...
**学习率错误。** 太高会让损失振荡，甚至直接跳到 NaN；太低则会让损失下降得很慢，看起来像没变化。Adam 通常从 1e-3 开始，SGD 通常从 1e-1 或 1e-2 开始。在下结论之前，最好先试 3 个相差 10 倍的学习率，例如 1e-2、1e-3、1e-4。

**死亡 ReLU。** 如果 ReLU 神经元收到很大的负输入，它就只会输出 0，而且梯度也是 0，之后再也激活不起来。如果死掉的神经元太多，网络就学不动了。检查方法：打印每个 ReLU 层后恰好为 0 的激活比例。如果超过 50%，就换成 LeakyReLU，或者先把学习率调低。

**梯度消失。** 在带有 sigmoid 或 tanh 激活的深层网络里，梯度在反向传播时会指数级缩小。等它传到第一层时，几乎已经是 0 了，第一层也就学不动了。修复方法：改用 ReLU/GELU，加入残差连接，或者使用批量归一化。
...
**数据泄露。** 测试数据混进了训练过程，结果准确率高得可疑。常见原因包括：在拆分前洗牌、预处理时用了整份数据集的统计量、不同数据划分之间有重复样本。修复方法是先拆分，再预处理，同时检查重复项。**标签错误。** 大多数真实数据集中有 5-10% 的标签是错的（Northcutt 等人，2021 - “测试集中普遍存在的标签错误”）。模型学到的其实是噪声。修复：用置信学习找出并修正标错的样本，或者用损失截断忽略高损失样本。
...
取一小批（8-32 个样本）。对它训练 100 多次。损失应该接近于零，训练准确率应该达到 100%。如果没有，说明你的模型或训练循环里有根本性错误，先别跑完整训练。
