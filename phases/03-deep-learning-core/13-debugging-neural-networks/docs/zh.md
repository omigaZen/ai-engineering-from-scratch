...
**类型:** 构建
**语言:** Python、PyTorch
**先修:** 第 03 阶段第 01-10 课（尤其是反向传播、损失函数、优化器）
**时长:** ~90 分钟
...
Leslie Smith（2017）提出，在一个训练轮次里把学习率从很小的值（`1e-7`）逐步扫到很大的值（`10`），同时记录损失。然后画出损失与学习率的关系图。最佳学习率通常比损失开始快速下降的位置小一个数量级。
...
这里有一个诊断工具包，用于监控激活、梯度和损失曲线。你会故意把网络弄坏，再用这个工具包逐个排查问题。
...
在编辑器里给行号栏打断点，用 Variables 面板查看张量属性。调试控制台可在运行时执行任意 Python 表达式。这对于数据预处理流水线这种“每一步都要看清楚”的场景特别有用。
